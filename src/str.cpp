...
struct motion getfromstr(String str){
  int num = 0; bool isMinas = false;
  struct motion answer;

  num = str.indexOf("L:") + 2;
  if((str[num]-48) == '-'){ isMinas = true; num++; }//L:-〇〇のときに出てきた値に-をかける。
  answer.l = (str[num]-48) * 100 + (str[num+1]-48) * 10 + (str[num+2]-48);
  if(isMinas == true){ answer.l = answer.l * -1; isMinas = false; }

  num = str.indexOf("R:") + 2;
  if((str[num]-48) == '-'){ isMinas = true; num++; }//R:-〇〇のときに出てきた値に-をかける。
